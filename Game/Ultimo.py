    ...
    def next_move(self, current_state):
        ...
        if pt_in_roi:
            ...
            if (current_state['puck_speed']['x'] < 0):
                if(target_pos['y']<140.8):
                    calculated_x=-10
                    target_pos['y']=140.8
                elif(target_pos['y']>371.2):
                    calculated_x=-10
                    target_pos['y'] = 371.2
                else:

                    y= abs(256-target_pos['y'])
                    calculated_x= (((115.2+50)**2)-(y**2))**(1/2)
            else:
                calculated_x=115.2+50
                target_pos['y']=256

            if (current_state['puck_pos']['x'] == 248.75):
                target_pos={'x': (target_pos['x']), 'y': target_pos['y']}
                self.kick_off=False
            else:
                target_pos = {'x': calculated_x, 'y': target_pos['y']}
